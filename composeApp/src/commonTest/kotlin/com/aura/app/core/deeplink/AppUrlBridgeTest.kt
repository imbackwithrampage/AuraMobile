package com.aura.app.core.deeplink
...
class AppUrlBridgeTest {
    ...
    fun `parses existing notification meta deeplink`() {
        assertEquals(
            ...
            parseAppDeepLink("aura://meta?type=series&id=tt0944947"),
        )
    }
    ...
    fun `parses direct nuvio addon install deeplink`() {
        assertEquals(
            ...
            parseAppDeepLink("aura://free.nebulapro.xyz/sports/i/free/manifest.json"),
        )
    }
    ...
    fun `parses direct imdb detail deeplink`() {
        assertEquals(
            AppDeepLink.Meta(type = "series", id = "tt0944947"),
            parseAppDeepLink("aura://series/tt0944947"),
        )
    }
    ...
    fun `parses provider imdb detail deeplink`() {
        assertEquals(
            AppDeepLink.Meta(type = "series", id = "tt0944947"),
            parseAppDeepLink("aura://imdb/series/tt0944947"),
        )
    }
    ...
    fun `parses provider tmdb detail deeplink`() {
        assertEquals(
            ...
            parseAppDeepLink("aura://tmdb/tv/1399"),
        )
    }

    @Test
    fun `does not treat reserved auth link as addon install`() {
        assertNull(parseAppDeepLink("aura://auth/trakt?code=abc"))
    }
